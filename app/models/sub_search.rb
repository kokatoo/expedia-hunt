class SubSearch < ActiveRecord::Base
  ...
  def load_flight(result, url)
  	...
  	flight.end = self.end.strftime("%d/%m/%Y")
  ...
  def load_flights
		...
		flight_url = "http://www.expedia.com/Flight-Search-All?action=FlightSearchAll%40searchFlights&origref=www.expedia.com%2FFlight-Search-All&inpFlightRouteType=2&inpDepartureLocations=#{CGI.escape self.source}&inpArrivalLocations=#{CGI.escape self.destination}&inpDepartureDates=#{CGI.escape self.start.strftime("%m/%d/%Y")}&inpArrivalDates=#{CGI.escape self.end.strftime("%m/%d/%Y")}&inpAdultCounts=1&inpChildCounts=0&inpChildAges=-1&inpChildAges=-1&inpChildAges=-1&inpChildAges=-1&inpChildAges=-1&inpInfants=2&inpFlightAirlinePreference=&inpFlightClass=3"
